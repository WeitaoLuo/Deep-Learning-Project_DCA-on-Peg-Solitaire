...
class DCAEnv(object):
	...
	def feasible_actions(self):
		...
		for i, pos in enumerate(GRID):
			if self.pegs[pos] == 0:  # if no peg, we cannot assume there was peg. Hence no action.
				actions[i, :] = False
			else:
				x, y = pos
	...
	def action_jump_feasible(self, pos, move_id):
		...
		d_x, d_y = MOVES[move_id]
		# MOVES = [(0, 1), (0, -1), (1, 0), (-1, 0)]
		return self.pegs[(x - d_x, y - d_y)] == 0 and self.pegs[(x - 2 * d_x, y - 2 * d_y)] == 0



	# render function is not yet changed
